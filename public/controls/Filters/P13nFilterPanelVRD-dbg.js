...
// Provides control controls.P13nFilterPanelVRD.
sap.ui.define([
	'./P13nConditionPanelVRD', 'sap/m/P13nPanel', 'sap/m/library', 'sap/m/Panel'
	 ...
	 */
	var P13nFilterPanel = P13nPanel.extend("controls.P13nFilterPanelVRD", /** @lends sap.m.P13nFilterPanel.prototype */
	{
		metadata: {
	...
	P13nFilterPanel.prototype.setConditions = function(aConditions) {
		...
		//if (aEConditions.length > 0) {
			this._oExcludePanel.setExpanded(false);
		//}
		return this;
	};
	...
	P13nFilterPanel.prototype.init = function() {
		...
		if (!this._aIncludeOperations["help"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "help");
		}
		if (!this._aIncludeOperations["multi"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ,
		...
		if (!this._aIncludeOperations["combo"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "combo");
		}
		if (!this._aIncludeOperations["comboTipoTicket"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboTipoTicket");
		}
		if (!this._aIncludeOperations["comboEstPla"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboEstPla");
		}
		if (!this._aIncludeOperations["comboPlataforma"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboPlataforma");
		}
		if (!this._aIncludeOperations["comboMetodologia"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboMetodologia");
		}
		if (!this._aIncludeOperations["comboTipoUsuario"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboTipoUsuario");
		}
		if (!this._aIncludeOperations["comboMolde"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboMolde");
		}
		if (!this._aIncludeOperations["comboTipoServicio"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboTipoServicio");
		}
		if (!this._aIncludeOperations["comboEstatusActvs"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "comboEstatusActvs");
		}
		...
		if (!this._aIncludeOperations["numeric2"]) {
			this.setIncludeOperations([
				sap.m.P13nConditionOperation.EQ
			], "numeric2");
		}
		...
		this._oIncludePanel.setHeaderToolbar(
			new sap.m.Toolbar({
				content : [
					new sap.m.Title({	
//						text : this._oRb.getText("FILTERPANEL_INCLUDES"),
						text : Utils.fnReadTranslate("FILTERPANEL_INCLUDES"),
					}).bindProperty("text", "i18n>FILTERPANEL_INCLUDES"),
					new sap.m.Title({	
						id : this._oIncludePanel.getId()+"_TitleCount",
						text : "(*)",
					})
				]
			})
		...
		this._oExcludePanel.setHeaderToolbar(
			new sap.m.Toolbar({
				content : [
					new sap.m.Title({	
						text : this._oRb.getText("FILTERPANEL_EXCLUDES"),
					}).bindProperty("text", "i18n>FILTERPANEL_EXCLUDES"),
					new sap.m.Title({	
						id : this._oExcludePanel.getId()+"_TitleCount",
						text : "(*)",
					})
				]
			})
		...
		var that = this;
		this.LimitRowsInput = new sap.m.Input({
			value	: "200",
			type	: "Number",
			width	: "100px",
			submit: function(oEvent){
				var oButtonSearch = that.getButton_Search();
				oButtonSearch.firePress();
			}
		})
		
		var oMaxLimit = new sap.ui.layout.VerticalLayout({
			content : [
				new sap.ui.layout.Grid({
					...
					content : [
						new sap.m.Label({
							text : Utils.fnReadTranslate("FILTERPANEL_INPUTMAX")
						}).bindProperty("text", "i18n>FILTERPANEL_INPUTMAX"),
						this.LimitRowsInput
					]
				})
			]
		...
		this.addAggregation("content", oMaxLimit);

		this._updatePanel();
	};
	
	P13nFilterPanel.prototype.getLimitRows = function() {
		
		var limit = this.LimitRowsInput.getValue();
		if(limit == "") limit = "0";
		return limit;
	},
	
	P13nFilterPanel.prototype.setButtonsToolbar = function(oBtn1, oBtn2) {
		this._oIncludePanel.getHeaderToolbar().addContent(oBtn1);
		this._oIncludePanel.getHeaderToolbar().addContent(oBtn2);
	},
	
	P13nFilterPanel.prototype.getButton_Search = function() {
		//return this._oIncludePanel.getHeaderToolbar().getContent()[2];
		return this.getParent().getHeaderToolbar().getContent()[2];
	},

	P13nFilterPanel.prototype.getButton_Reset= function() {
		//return this._oIncludePanel.getHeaderToolbar().getContent()[3];
		return this.getParent().getHeaderToolbar().getContent()[3];
	},

	P13nFilterPanel.prototype.Reset= function() {
		this._oIncludePanel.getHeaderToolbar().getContent()[3].firePress();
	},
		...
		if (this._bUpdateRequired) {
			...
			this.getItems().forEach(function(oItem_) {
				...
				aKeyFields.push({
					...
					zfunction: oContext.getObject()["zfunction"]
				});
