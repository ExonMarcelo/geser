...
// Provides control controls.P13nConditionPanelVRD.
sap.ui.define([
		'jquery.sap.global', 'sap/m/library', 'sap/ui/core/Control', 'sap/ui/core/format/DateFormat', 'sap/ui/core/format/NumberFormat', 'sap/ui/core/IconPool', 'sap/ui/Device', 'sap/ui/core/InvisibleText', 'sap/ui/core/library', 'sap/ui/core/ResizeHandler', 'sap/ui/core/Item'
	 ...
	 * @alias controls.P13nConditionPanelVRD
	 * @ui5-metamodel This control/element also will be described in the UI5 (legacy) designtime metamodel
	 */
	var P13nConditionPanel = Control.extend("controls.P13nConditionPanelVRD", /** @lends sap.m.P13nConditionPanel.prototype */
	{
		metadata: {
	...
	P13nConditionPanel.prototype.setConditions = function(aConditions) {
		if (!aConditions) {
			jQuery.sap.log.error("controls.P13nConditionPanelVRD : aCondition is not defined");
		}
			...
			for ( var conditionId in this._oConditionsMap) {
				...
				if (!oCondition._oGrid || oCondition._oGrid.select.getSelected()) {
					...
					var aFilter = {
						...
						"value2": oCondition.operation === sap.m.P13nConditionOperation.BT ? oCondition.value2 : "",
						"showIfGrouped": oCondition.showIfGrouped,
						"typeField": oCondition.typeField
					};
					//ASVRAMIREZ
					if(aFilter.keyField == "Mes"){
						if(typeof aFilter.value1 != "number")
							aFilter.value1 = ( loData .getMonths("abbreviated").indexOf(aFilter.value1) + 1);
					}
					
					switch(oCondition.operation){
						case "EQ":							
							if(aFilter.value1!="")
								aFilter.zstr = aFilter.keyField+" = "+aFilter.value1;
							break;
						case "StartsWith":
							aFilter.zstr = aFilter.keyField+".StartsWith("+aFilter.value1+")";
							break;
						case "EndsWith":
							aFilter.zstr = aFilter.keyField+".EndsWith("+aFilter.value1+")";
							break;
						case "Contains":
							aFilter.zstr = aFilter.keyField+".Contains("+aFilter.value1+")";
							break;
						case "BT":
							aFilter.zstr = aFilter.keyField+" ge "+aFilter.value1+" and "+aFilter.keyField+" le "+aFilter.value2;
							break;
						case "LT": case "LE":
							aFilter.zstr = aFilter.keyField+" le "+aFilter.value1;
							break;
						case "GT": case "GE":
							aFilter.zstr = aFilter.keyField+" ge "+aFilter.value1;
							break;
					}
					
					aConditions.push(aFilter);
				}
			}
		}
	...
	P13nConditionPanel.prototype.init = function() {
		...
		*/
		this._sFromLabelText 	= Utils.fnReadTranslate("CONDITIONPANEL_LABELFROM");
		this._sToLabelText 		= Utils.fnReadTranslate("CONDITIONPANEL_LABELTO");
		this._sValueLabelText 	= Utils.fnReadTranslate("CONDITIONPANEL_LABELVALUE");
		this._sShowIfGroupedLabelText 		= Utils.fnReadTranslate("CONDITIONPANEL_LABELGROUPING");
		this._sValidationDialogFieldMessage = Utils.fnReadTranslate("CONDITIONPANEL_FIELDMESSAGE");
		...
		if (!this._oPaginatorToolbar) {
			if (iItems > this._iConditionPageSize) {
				...
				if (oParent && oParent.setHeaderText) {
					...
					try{
						var idParent = oParent.getId();
						sap.ui.getCore().byId(idParent+"_TitleCount").setText(iItems > 0 ? " (" + iItems + ")" : "");
					}
					catch(ex){
						console.log(ex);
					}
	...
	P13nConditionPanel.prototype._createValueField = function(oCurrentKeyField, oFieldInfo, oConditionGrid) {
		...
		var params = {
			...
			submit: function(oEvent){
				//oCnt_Upload.OnFilterSearch();
				try{
					//var oButtonSearch = oEvent.getSource().getParent().getParent().getParent().getParent().getParent().getParent().getParent().getFooter().getContentMiddle()[0]
					//var oButtonSearch = oEvent.getSource().getParent().getParent().getParent().getParent().getHeaderToolbar().getContent()[2];
					var oButtonSearch = oEvent.getSource().getParent().getParent().getParent().getParent().getParent().getParent().getHeaderToolbar().getContent()[2];
					oButtonSearch.firePress();
				}
				catch(e){
					console.log(e);
				}
		...
		switch (sCtrlType) {
			...
			case "numeric":
				...
				break;
			case "combo":
				//oConditionGrid.oFormatter = DateFormat.getDateInstance({pattern : "yyyyMMdd" });
				...
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mCombos>/ESTATUS",
						template: new sap.ui.core.ListItem({
							key:"{mCombos>Id}", text:"{mCombos>Descripcion}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			        ...
			        // Your code...
			        setTimeout(function(){ that._changeField(oConditionGrid); }, 250);
				}
			...
			case "combo1":
				...
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "X",
					items: {
						path: "mOrganizacionVentas>/d/results",
						template: new sap.ui.core.ListItem({
							key:"{mOrganizacionVentas>SalesOrgId}", text:"{mOrganizacionVentas>SalesOrgDesc}"
						})
					}
				});
			...
			case "comboTipoTicket":
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mCCatalogos>/TIPOS_TICKETS",
						template: new sap.ui.core.ListItem({
							key:"{mCCatalogos>campo1}", text:"{mCCatalogos>campo3}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "comboPlataforma":
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mCCatalogos>/PlataformasActivas",
						template: new sap.ui.core.ListItem({
							key:"{mCCatalogos>id_plataforma}", text:"{mCCatalogos>descripcion_plataforma}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "comboTipoUsuario":
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mTipoUsuarioSegUsuario>/TIPOS_USUARIO",
						template: new sap.ui.core.ListItem({
							key:"{mTipoUsuarioSegUsuario>id_tipo_usuario}", text:"{mTipoUsuarioSegUsuario>descripcion}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "comboEstPla":
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mCCatalogos>/EstatusPlataforma",
						template: new sap.ui.core.ListItem({
							key:"{mCCatalogos>id_estatus}", text:"{mCCatalogos>descripcion_estatus}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "comboMolde":
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mCCatalogos>/MoldexUsuLog",
						template: new sap.ui.core.ListItem({
							key:"{mCCatalogos>id_molde}", text:"{mCCatalogos>descripcion_empresa} - {mCCatalogos>descripcion_molde}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "comboTipoServicio":
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mCCatalogos>/TipoServicioxUsuLog",
						template: new sap.ui.core.ListItem({
							key:"{mCCatalogos>id_tipo_servicio}", text:"{mCCatalogos>descripcion_empresa} - {mCCatalogos>descripcion_tipo_servicio}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "comboEstatusActvs":
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mContactos>/ESTATUSACTVSVOBO",
						template: new sap.ui.core.ListItem({
							key:"{mContactos>id_estatus}", text:"{mContactos>descripcion}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "multi":
				oControl = new sap.m.MultiInput(params);
				oControl.setShowValueHelp(true);
				oControl.setValueHelpOnly(false);
				oControl.attachValueHelpRequest(function(oEvent) {
					if(oCurrentKeyField.zfunction != undefined)
						oCurrentKeyField.zfunction( oEvent.getSource(), function() { setTimeout(function(){ that._changeField(oConditionGrid); }, 250); } );
					else
						//P13NDIALOG_VALIDATION_MESSAGE - MESSAGE_PAGE_DESCRIPTION - MESSAGE_PAGE_TEXT
						alert(that._oRb.getText("MESSAGE_PAGE_DESCRIPTION")+"\n"+that._oRb.getText("MESSAGE_PAGE_TEXT"));
				});
				break;
			case "help":
				oControl = new sap.m.Input(params);
			...
			case "date":
				...
				oControl.setDisplayFormat("yyyy-MM-dd");
			...
			case "month":
				...
				oControl.attachValueHelpRequest(function(oEvent) {
					...
					var oDialog = new sap.m.Dialog({
						...
						///contentWidth: "100%",
						content: 
							new sap.ui.unified.calendar.MonthPicker("MP1",{
						...
						buttons:[
							new sap.m.Button({
								text: '{i18n>sap.m.MessageBox.Action.Ok}',
								press: function (oEvt) {
									var oMP = oCore.byId("MP1");
									var iMonth = oMP.getMonth();
									var oMonths = oMP._oLocaleData.mData["ca-gregorian"].months.format.abbreviated;
									
									oInput.setValue( oMonths[iMonth] );
									oDialog.close();
								}
							}),
							new sap.m.Button({
								text: '{i18n>view.messagedialog.cerrar}',
								press: function (oEvt) {
									oInput.setValue("");
									oDialog.close();
								}
			...
			case "valueHelpUsuariosTR":
				...
				oControl.attachValueHelpRequest(function(oEvent) {
					// Dialog
					var oInput = oEvent.getSource();
					var oModel= oCore.getModel("mUsuariosTR");
					
					var lista_usuarios= new sap.m.StandardListItem({	
						title:"{mUsuariosTR>usuario}",
						description:"{mUsuariosTR>nombre_completo}"
					});
					
					var selectUsuariosTR = new sap.m.SelectDialog({
						noDataText: oCore.getModel("i18n").getProperty("No-Found"),	
						title: oCore.getModel("i18n").getProperty("SelectUsuarioTR.Title"),
							confirm:function(oEvt)
							{
								...
								if (oSelectedItem) {
									...
									var sPath = oEvt.getParameter("selectedItem").getBindingContext("mUsuariosTR").getPath();
									var oObj = oCore.getModel("mUsuariosTR").getProperty(sPath)
									
									oInput.setValue(oObj.usuario);
									oInput.updateInputField();
									setTimeout(function(){ that._changeField(oConditionGrid); }, 250);
							...
							search:function(oEvt)
							{
								...
								var oFilterUsuario = new Filter("usuario", sap.ui.model.FilterOperator.Contains, sValue);
								var oFilterNombre = new Filter("nombre_completo", sap.ui.model.FilterOperator.Contains, sValue);
								var oModelFilter = new sap.ui.model.Filter([oFilterUsuario, oFilterNombre]);
								var oBinding = oEvt.getSource().getBinding("items");
								oBinding.filter(oModelFilter, false);
							}
					});
					
					selectUsuariosTR.setModel(oModel,"mUsuariosTR");
					selectUsuariosTR.bindAggregation("items","mUsuariosTR>/USUARIOS_TR", lista_usuarios);
					
					function f_openselectUsuariosTR(){
						selectUsuariosTR.open();
					};
					oCtrl_MonitorTR.oServ_UsuariosTR( f_openselectUsuariosTR);
				});
				break;
			case "year":
				oControl = new sap.m.Input(params);
						...
						content: 
							new sap.ui.unified.calendar.YearPicker("YP1",{
								...
								select: function(oEvent){/*
									...
									var iYear = oYP.getYear();
									oInput.setValue(iYear);*/
								}
							}),
						buttons:[
							new sap.m.Button({
...
//				}
	...
	P13nConditionPanel.prototype._fillOperationListItems = function(oCtrl, aOperations, sType) {
		...
		for ( var iOperation in aOperations) {
			var sText;
			if(this._oRb.hasText("CONDITIONPANEL_OPTION" + sType + aOperations[iOperation])){
//				sText = this._oRb.getText("CONDITIONPANEL_OPTION" + sType + aOperations[iOperation]);
				sText = Utils.fnReadTranslate("CONDITIONPANEL_OPTION" + sType + aOperations[iOperation]);
			}
			else {
				// when for the specified type the resource does not exist use the normal string resource text
				switch( aOperations[iOperation] ){
					case "CP":
						sText = this._oRb.getText("CONDITIONPANEL_OPTIONContains");
						break;
					case "CPL":
						sText = this._oRb.getText("CONDITIONPANEL_OPTIONStartsWith");
						break;
					case "CPR":
						sText = this._oRb.getText("CONDITIONPANEL_OPTIONEndsWith");
						break;
					default:
	...
	P13nConditionPanel.prototype._changeField = function(oConditionGrid, oEvent) {
		...
		if(sCtrlType === "month") {
			//var lo = new sap.ui.core.Locale( oCore.User.Language.toUpperCase() );
			//var loData = new sap.ui.core.LocaleData(lo);
			
			oValue1 = ( loData .getMonths("abbreviated").indexOf(sValue1) + 1); 
			oValue2 = "";
		}
