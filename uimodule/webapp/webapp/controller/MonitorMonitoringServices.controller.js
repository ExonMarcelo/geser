var oCtrl_MonMonitoringServices;
var oFragments = {};
sap.ui.define(["jquery.sap.global", 
	...
	return Controller.extend("com.exonmarcelo.geser.webapp.controller.MonitorMonitoringServices", {

		onInit : function(oEvt) {
			oCtrl_MonMonitoringServices = this;
			console.log("init");
			
			oView = oCtrl_MonMonitoringServices.getView();
			...
			Utils.fnCreateFilterPanel("MONITORING_SERVICES", oCtrl_MonMonitoringServices)

			//oCtrl_Menu.fnInsertUserBar(oCtrl_MonMonitoringServices, "idPageMonitorCustomers");
		},
		onBeforeShow:function(oEvt){
			oComponent.fnToggleButtonsTP(true);
			oComponent.fnToggleButtonUser(true);
			oComponent.fnChangeTitleApp("ToolPage.Header.ToolHeader.Title.8");
		},
		onAfterRendering : function(){
			//oCtrl_MonMonitoringServices.byId("idTitleUserBar").setText("Monitor de Clientes");
		},
			...
			var oSettings = {
				...
				fnLocalFilter:function(){
					...
					if(aFiltersToSend.length > 0){
						...
						var oBinding = oCtrl_MonMonitoringServices.byId("idTableCustomers").getBinding("items");
		...
		fnCleanFilters:function(){
			Utils.fnCleanTableFilters(oCtrl_MonMonitoringServices, "idTableCustomers");
		},
		...
		onItemPress:function(oEvt){
			...
			var router = sap.ui.core.UIComponent.getRouterFor(oCtrl_MonMonitoringServices);
		...
		onSettingsTableCustomers: function(oEvt){
			...
			var viewSettings = new sap.m.ViewSettingsDialog({
				...
				confirm: function(oEvent){
					var oTable = oCtrl_MonMonitoringServices.byId("idTableCustomers"),
					...
					oCtrl_MonMonitoringServices.byId("idCounterCustomers").setText("("+oBinding.aIndices.length+")");
